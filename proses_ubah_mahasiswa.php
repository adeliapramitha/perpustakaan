...
if($_POST){
    ...
    if(empty($nama)){
        ...
        include "koneksi.php";
    if(empty($password)){
        $update=mysqli_query($conn, "update mahasiswa set nama='".$nama."', tanggal_lahir='".$tanggal_lahir."',  jenis_kelamin='".$jenis_kelamin."',alamat='".$alamat."', 
        username='".$username."', id_jurusan='".$id_jurusan."'where id_mhs = '".$id_mhs."' ") or die(mysqli_error($conn));
    if($update){
        ...
        $update=mysqli_query($conn, "update mahasiswa set nama='".$nama."', tanggal_lahir='".$tanggal_lahir."', jenis_kelamin='".$jenis_kelamin."', alamat='".$alamat."', 
        username='".$username."', password='".$password."', id_jurusan='".$id_jurusan."'where id_mhs = '".$id_mhs."' ") or die(mysqli_error($conn));
    if($update){
        echo "<script>alert('Sukses update data');location.href='tampil_mahasiswa.php';</script>";
    }else {
        echo "<script>alert('Gagal update data');location.href='ubah_mahasiswa.php?id_mhs=".$id_mhs."';</script>";
